...
import {RequestStatusType, setAlertMessageAC, setLoadingStatusAC} from '../App/appReducer';
import {setAuthProfileAC, setProfilePhotoAC} from '../Auth/authReducer';
...
export const changeProfilePhotoThunkCreator = (photo: File) => (dispatch: Dispatch) => {
    dispatch(setLoadingStatusAC('loading'))
    dispatch(setProfileEntityAC('loading'))
    profileAPI.updatePhoto(photo)
        .then((res) => {
            if (res.data.resultCode === 0) {
                ...
                dispatch(setLoadingStatusAC('succeeded'))
                dispatch(setProfileEntityAC('succeeded'))
                setAlertMessageAC('Profile photo Successfully!')

            } else {
                handleServerAppError(res.data, dispatch)
                setProfileEntityAC('failed')
            }
        })
        .catch((e) => {
            handleServerNetworkError(dispatch, e)
